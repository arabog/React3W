...
Think of memoization as caching a value so that it does not need to 
be recalculated.
...
In this example, you might think that the Todos component will 
not re-render unless the todos change:
...
const App = () => {
          ...
          // we can replace setState fxn with another fxn
          const increment = () => {
                    setCount((c) => c + 1);
          };
...
You will notice that the Todos component re-renders even when the 
todos do not change.

Why does this not work? We are using memo, so the Todos 
component should not re-render since neither the todos state 
...
Use the useCallback Hook to prevent the Todos component from 
